...
// getCtx
// Used as test context for session handlers
...
func TestRepository_Success_QueryResult(t *testing.T) {
	...
	// put in session 'userId' and 'query'
	var u int = 1

	req, _ := http.NewRequest("GET", "/result?query=test+query", nil)
	ctx := getCtx(req)
...
func TestRepository_Redirect_Without_UserId_And_Query_In_Session_QueryResult(t *testing.T) {
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("failed get /result without 'userId' and 'query' in session, expected status %d, but was: %d", http.StatusTemporaryRedirect, rr.Code)
	}
}
...
func TestRepository_Success_WithoutQueryInSession_QueryResult(t *testing.T) {
	...
	if rr.Code != http.StatusOK {
		t.Errorf("failed get /result without 'userId' and 'query' in session, expected status %d, but was: %d", http.StatusOK, rr.Code)
	}
}

func TestRepository_Redirect_Error_From_DB(t *testing.T) {
	// userId must be equal to 5 to return error from test_repo on GetUserSearchesByUserIdAndPartialTextQuery DB query
	var u int = 5
	...
	if rr.Code != http.StatusSeeOther {
		t.Errorf("failed get /result, expected status %d, but was: %d", http.StatusSeeOther, rr.Code)
	}
}
